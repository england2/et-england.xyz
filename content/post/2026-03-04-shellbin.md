...
- We define a seperate Helm values file than what shellbin ships with 
...
However, Helm is only acting as a yaml templating engine [^1]. It helps us produce yaml, but this resulting yaml has no knowledge of our container images aside containing a string that provides a url to fetch a certain image.
...
[^1]: Additionally, in our case we don't even deploy a Helm Chart. ArgoCD uses the Helm Templating engine to render and deploy yaml, and then manages the resulting resources itself. Therefore, `$ helm [upgrade|rollback|uninstall|etc]` are all inert on `shellbin` because Helm is not managing that yaml, ArgoCD is.
...
Moving on, here's a thoughr process on designing our pipeline:
...
After we commit and push, here's what happens:
...
- `helm-values-shellbin.yml`
- ArgoCD sees that the deployed yaml is different than the 
